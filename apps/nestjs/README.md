> **Note:** This is the NestJS API application within a larger `pnpm` monorepo. For project-level setup and commands, please see the [root README.md](../../README.md).
...
# watch mode (from this directory, if needed)
$ pnpm run start:dev

# production mode (from monorepo root)
$ pnpm build
```
...
```bash
# unit tests (from monorepo root)
$ pnpm test

# e2e tests (from this directory)
$ pnpm run test:e2e

# test coverage (from this directory)
$ pnpm run test:cov
```
...
2. Generate migration:
  pnpm --filter @thrive/api migration:generate -- src/migrations/DescriptiveName
3. Review the generated file (ensure no unintended WordPress table changes)
4. Run migrations:
  pnpm --filter @thrive/api migration:run
5. Revert last (if needed):
  pnpm --filter @thrive/api migration:revert
