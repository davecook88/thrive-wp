...
import { ServiceType } from '../common/types/class-types';
...
export class GroupClassesService {
  ...
  async generateSessions(groupClassId: number): Promise<Session[]> {
    const groupClass = await this.groupClassesRepository.findOne({ where: { id: groupClassId } });
  ...
  async getAvailableSessions(filters: { levelId?: number; teacherId?: number; startDate?: Date; endDate?: Date }): Promise<any[]> {
    ...
    if (filters.startDate) {
      qb.andWhere('session.startAt >= :startDate', { startDate: filters.startDate });
    }
    ...
    if (filters.levelId) {
      qb.andWhere('groupClass.levelId = :levelId', { levelId: filters.levelId });
    }

    if (filters.teacherId) {
      qb.andWhere('session.teacherId = :teacherId', { teacherId: filters.teacherId });
    }
    ...
    return sessions.map(session => ({
      ...session,
      availableSpots: session.capacityMax - (session as any).enrolledCount,
      isFull: (session as any).enrolledCount >= session.capacityMax,
      canJoinWaitlist: (session as any).enrolledCount >= session.capacityMax,
    }));
  }
}
