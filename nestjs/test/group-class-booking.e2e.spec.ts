...
describe('Group Class Booking (e2e)', () => {
  ...
  describe('POST /bookings', () => {
    it('should allow a student to book a group class session', async () => {
      ...
      const booking = await bookingRepository.findOne({ where: { id: bookingRes.body.bookingId } });
      ...
      const updatedSessionsRes = await request(app.getHttpServer())
        .get('/group-classes/available')
        .expect(200);
      const updatedSession = updatedSessionsRes.body.sessions.find(s => s.sessionId === sessionToBook.sessionId);
    ...
    it('should not allow booking a full group class session', async () => {
      ...
      const secondUser = await userRepository.save({ email: 'student2@test.com', firstName: 'Second', lastName: 'Student', passwordHash: 'hash' });
      await request(app.getHttpServer())
        .post('/bookings')
